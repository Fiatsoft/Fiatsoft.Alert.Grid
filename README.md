...
- **Specify a default action-command for all messages**: For more advanced useage.
...
2. Specify message-filter:
   ...
   - Enter a command to pass the message to Fiatsoft.Alert.Grid.exe compatible with FiltaQuilla (comma,seperated) syntax, using `support@fiatsoft.com` as an example (replace with the name of your inbox); and adjust the path to your installed Fiatsoft.Alert.Grid.exe and Thunderbird.exe as neccessary:
...
Runs as a daemon/messager. The daemon listens for alert-items, and the messager sends them. Although there is clear seperation and order between these modes & options, the program is designed to correct improper usage. The mode is specified with `-mode` as `push` (messenger) or `pull` (daemon). Each daemons listens for messages belong to one `-topic`, and no other daemon can listen to that same `-topic` (would exit silently).
...
- `-mode` Can be `push` or `pull` for a daemon/messager modes, respectively, as documented below (optional, defaults to `pull`).
...
- `-schema`
  The accepted column-names from the recieved Alerts to display, specified as a JSON-array (optional, defaults to inferring schema from first received Alert).
- `-title`
...
- `-size`
  The width and height of the window in `WidthxHeight` format (optional, superseeds -width/-height arguments).
- `-defaultAction`
...
- `-dateFormat`
  The date format to apply to the 'Created' default-column (optional: defaults to `O`, the system default).
- `-hideCreated`
  Hide the above-previous 'Created' column.

**Messenger Arguments:**

- `-action` A command to run with clicking on `Run` assoiciated with this item (optional).
...
```cmd
Fiatsoft.Alert.Grid.exe -mode push -Topic "Thunderbird: support@fiatsoft.com" -action """C:\Program Files\Mozilla Thunderbird\thunderbird.exe"" mid:a3a5ee0f-8275-e09e-4cbf-59183a4a5805@fiatsoft.com" From DragonGrrrrl@gmail.com Subject "Love letterrr"
```
...
Based on design of Microsoft Outlook's "New Mail Alerts"/'New Item Alert' window, which is commonly programmed to display upon recieving new mail and remains on top of the Desktop until dismissed (unlike the modern Widows notification system, which offers only an un-numbered notification icon for missed alerts):  
...
2. [Newtonsoft.Json](https://www.newtonsoft.com/json)
   - **NuGet**: [Newtonsoft.Json NuGet](https://www.nuget.org/packages/Newtonsoft.Json)
...
3. [WixToolset.UI.wixext](https://www.nuget.org/packages/WixToolset.UI.wixext/5.0.2?_src=template)
   ...
   - For using **Wix Toolset** (the general toolkit, including the CLI) and **Wix-related extensions by FireGiant**, you'll need to acknowledge both, and it's important to refer to their specific licensing terms.
