...
namespace Fiatsoft.Alert.Grid {
    public partial class App : Application {
        ...
        readonly static string assembly = Assembly.GetExecutingAssembly()?.GetName()?.Name ?? "Fiatsoft.Alert.Grid";
        readonly Dictionary<string, string> Values = [];
        private NamedPipeServerStream? pipeServer;
        private NamedPipeClientStream? pipeClient;

        public string DefaultTitle { get; } = $"{assembly} v{Assembly.GetExecutingAssembly().GetName().Version}";
        ...
        public object? TopicNameSafe { get; private set; }
            ...
            try {
                for (int i = 1; i < args.Length; i++) {
                    ...
                    if (Fiatsoft.Alert.Grid.GeneratedRegex.Mode().IsMatch(arg))
                        ...
                        HideCreatedColumn = bool.Parse(args[(i += 1)]);
                    }
                    else {
                        if (args.Length<= i+1)
                ...
                MessageBox.Show(HelpContent, assembly);
                Shutdown(0);
            }
            catch (Exception ex) {
                HandleException(ex);
        ...
        protected override void OnStartup(StartupEventArgs e) {
            ...
            TopicNameSafe = $"{assembly[Math.Max(0, assembly.Length - 260 - 1 - 6 - TopicName.Length)..]}.{TopicName}";
            ...
            var thread = new Thread(() => {
                if (Mode == "push") {
                    if (!IsDaemonRunning()) {
                        ...
                        Process.Start(Environment.ProcessPath ?? $".{Path.PathSeparator}", PrepCLIArgs(modifiedArgs));
                        try {
                    ...
                    try {
                        if (!TryRunExclusively(MutexName, () => { RunAsDaemonAsync(); }))
                            HandleDaemonAlreadyRunning(); //run as messenger; daemon already running (if message prescripted)
                    }
                    catch (Exception ex) {
                        HandleException(ex);
                    }
        ...
        private bool TryRunExclusively(string mutexName, Action action) {
            using (mutex = new Mutex(false, mutexName, out bool createdNew)) {
                ...
                return false; //Mutex lock stolen in likely race-condition vs another process/thread, recently spawning daemon
            }
        }

        private void RunAsDaemonAsync() {
            if (EventWaitHandle.TryOpenExisting($"{TopicNameSafe}.{"Launched"}", out var waitHandle)) {
                waitHandle.Set();
            }

            Schema = Schema.Concat([.. Values.Keys]).Distinct().ToList();
        ...
        private void InitializeContextMenu() {
            ...
            var showHideMenuItem = new System.Windows.Controls.MenuItem { Header = "Show/Hide Window"};
        ...
        private void PlayAlert() {
            try {
                ...
                soundPlayer.Play(); //player.PlaySync();
                soundPlayer.Dispose();
            }
            catch (Exception) {
                try {
        ...
        private static void HandleException(Exception ex) {
            MessageBox.Show(@$"Error: {ex.Message}: {ex.StackTrace}", assembly);
        }
        ...
        [Serializable]
        internal class UnmatchedKVPException : Exception {
